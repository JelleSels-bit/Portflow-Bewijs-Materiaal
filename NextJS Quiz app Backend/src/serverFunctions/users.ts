...
import {createUser, deleteUser, getUserByEmail, startSession, stopSession, updateUser} from '@/dal/users'
...
export const signInAction = publicFormAction({
  ...
  serverFn: async ({data, logger}) => {
    ...
    logger.info(`Sign in attempt for email: ${data.email}`)
    if (!user) {
...
export const updateProfileAction = protectedFormAction({
  ...
  serverFn: async ({data, profile}) => {
    ...
    // Het profiel wordt gebruikt in de DevNav component, aangezien deze component op de homepagina staat moeten we
...
export const adminUpdateUserAction = protectedFormAction({
  ...
  serverFn: async ({data, logger}) => {
    ...
    const operationName = 'Admin update user action'
    let success = false
    ...
    try {
      ...
      logger.info(`Updating user with id: ${data.id}`)
    ...
    if (success) {
      logger.info(`Redirecting to /userCRUD/put/${id}`)
      revalidatePath(`/userCRUD/put/${id}`, 'page')
    }
  },
...
export const adminDeleteUserAction = protectedFormAction({
  ...
  serverFn: async ({data, logger}) => {
    const startTime = Date.now()
    const operationName = 'Admin delete from user action'

    try {
      logger.info(`Operation started: ${operationName}`)

      if (!data.id) {
        logger.warn(`No user found with id: Id is missing`)
      ...
      logger.info(`User ${data.id} deleted successfully`)

      revalidatePath('/userCRUD/index', 'layout')
      redirect('/userCRUD/index/UsersPage')
    } catch (error) {
      logger.error(`Error in operation ${operationName}: ${(error as Error).message}`)
      throw error
    } finally {

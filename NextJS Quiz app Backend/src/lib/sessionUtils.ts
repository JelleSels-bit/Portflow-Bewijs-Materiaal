...
// change userId: tokenBody.sub -> userId: tokenBody.id
// change id: tokenBody.sub -> id: tokenBody.id,

export async function getSessionProfileFromCookie(stateful = true): Promise<PublicUser | null> {
  const session = await getSessionFromCookie(stateful)
  if (!session?.user) return null

  const {id, email, username, role} = session.user

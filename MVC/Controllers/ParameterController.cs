...
namespace Restaurant.Controllers
{
    ...
    public class ParameterController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            {
                Parameters = parameters.Select(p => new ParameterViewModel
                {
                    Id = p.Id,
                    Naam = p.Naam,
                    Waarde = p.Waarde
        ...
        public async Task<IActionResult> Edit([Bind(Prefix = "EditParameter")] ParameterEditViewModel model)
        {
            if (!ModelState.IsValid)
            {
                return RedirectToAction(nameof(WithError), new
                {
                    message = "Waarde ongeldig — wijziging niet opgeslagen."
                });
            }
            ...
            try
            {
                ...
                return RedirectToAction(nameof(WithSuccess), new { message = "Wijziging opgeslagen." });
            }
            catch
            {
                return RedirectToAction(nameof(WithError), new
                {
                    message = "Opslagfout — waarde blijft ongewijzigd."
                });
            }
        }

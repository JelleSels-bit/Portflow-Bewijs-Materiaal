...
//            ViewBag.ActiveTab = (tab ?? "beheren").ToLowerInvariant();

//            return View(viewModel);
//        }

//        // /Tafel/Toewijzen -> gewoon tweede tab tonen (voor later)
//        [Authorize(Roles = "Zaalverantwoordelijke,Eigenaar")]
//        public IActionResult Toewijzen()
//        {
//            return RedirectToAction(nameof(Index), new { tab = "toewijzen" });
//        }
...
namespace Restaurant.Controllers
{
    ...
    public class TafelController : Controller
    {
        ...
        public async Task<IActionResult> Toewijzen(DateTime? datum, bool alleenZonderTafel = false)
        {
            ...
            var vmList = _mapper.Map<List<TafelToewijzenReservatieViewModel>>(reservaties);
        ...
        public async Task<IActionResult> ToewijzenReservatie(int reservatieId)
        {
            ...
            var vm = _mapper.Map<TafelToewijzenReservatieViewModel>(reservatie);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> ToewijzenReservatie(TafelToewijzenReservatieViewModel model)
        {
        ...
        {
            var reservaties = await _context.ReservatieRepository
                .GetAllActiveAsync();
            ...
            var overzicht = reservaties
                ...
                .GroupBy(x => x.TafelNummer)
                .Select(g => new AfrekenenViewModel
                {
                    TafelNummer = g.Key,
                    Reservaties = g.Select(x => new ReservatieItemViewModel
                    {
                        Id = x.Reservatie.Id,
                        Naam = x.Reservatie.CustomUser?.UserName ?? "Onbekend",
                        Datum = x.Reservatie.Datum,
                        IsBetaald = x.Reservatie.Betaald,
                        Totaal = x.Reservatie.Bestellingen.Where(x => x.StatusId != 5 && x.StatusId != 4).Sum(b =>
                        {
                            if (b.Product == null)
                                return 0;
                            var prijsItem = b.Product.PrijsProducten?
                            ...
                            if (prijsItem == null)
                                return 0;
                            return prijsItem.Prijs * b.Aantal;
                        })
                    }).ToList()
                })
                .ToList();

...
export class PlayersPage {
    ...
    private validatePlayerName = (name: string): boolean => {
        ...
        if(quiz.players.some(p => p.name.toLowerCase() === name.toLowerCase()) ) {
            displayAlert(`Player name must be unique`,3000)
            return false
        }else if (name === "")
        {
            displayAlert("Please enter a player name",3000)
            return false;
        }
        return true;
    ...
    private addPlayer() {
        const input: HTMLInputElement = getElementWrapper("#input-player");
        const name = input.value;

        if(!this.validatePlayerName(name)) return;
        quiz.addPlayer(name);
        this.checkIfAllPlayersAreAdded()
        this.updatePlayerList();
        input.value = "";
    ...
    private checkIfAllPlayersAreAdded() {

        if (quiz.getNumberOfPlayers() === quiz.players.length) {
            enableEl(getElementWrapper("#btn-go-to-questions"))
            disableEl(getElementWrapper("#btn-add-player"))
        }
